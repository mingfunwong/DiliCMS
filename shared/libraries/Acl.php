						 ...
						 foreach ($v['sub_menus'] as $j)
						 {
						   ...
						   if ($this->_current_menu ==  2) {
						        echo '<li class="' . (isset($j['current']) ? 'selected' : '') .'"><a href="' . 
					   			...
					   			if (!$j['current'] && isset($_GET['model']) && $_GET['model'] == $j['extra'])
					   			{
					   				$j['current'] = true;
						   		}
						   		if (!(stripos($j['extra'], '_single') === false)) {
							   		echo '<li class="' . (isset($j['current']) ? 'selected' : '') .'"><a href="' . backend_url($j['class_name'] . '/form', $extra . '&id=1') . '">' . $j['menu_name'] . '</a></li>';
							   	} else {
