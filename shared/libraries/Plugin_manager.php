...
class Plugin_manager {
    ...
    {
        //加载插件
        if ($this->app->platform->cache_exists(CMS_SHARE_PATH . 'settings/plugins.php'))
		{
			eval('?>' . $this->app->platform->cache_read(CMS_SHARE_PATH . 'settings/plugins.php'));
    ...
    private function autoloader($class_name) 
    {
        ...
        $path = CMS_EXTENSION_PATH . $this->classmap[$class_name];
...
abstract class CMS_Plugin_Controller
{
	...
	public function __construct($name)
	{
		...
		$this->path = CMS_EXTENSION_PATH . 'plugins/' . $this->name . '/';
