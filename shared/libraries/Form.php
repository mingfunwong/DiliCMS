...
class Form 
{
	...
	{
		return '<input class="normal" name="' . $field['name'] . '" id="' .$field['name'] . 
		       '" type="text" style="width:100px" autocomplete="off" value="' . $default . '" />';	
	}
    ...
    {
        $field['width'] =  $field['width'] ? $field['width'] : 300;
        return '<input class="normal" name="' . $field['name'] . '" id="' . $field['name'] . 
    ...
    private function _radio($field, $default)
    {
        ...
        foreach ($field['values'] as $key=>$v)
        {
            $return .= '<li><input id="rad_' . $field['name'] . '_' . $count . '" name="' . $field['name'] . '" type="radio" value="' . 
                       $key . '" ' . ($default == $key ? 'checked="checked"' : '') . ' /><label class="attr" for="rad_' . $field['name'] . '_' . $count . '">' . $v . '</label></li>';
            $count ++;
        }
        $return .= '</ul>';
        return $return;
    ...
    private function _checkbox($field, $default)
    {
        ...
        if (is_array($field['values']))
        {
            ...
            foreach ($field['values'] as $key => $v)
            {
                $return .= 	'<li><input id="chk_' . $field['name'] . '_' . $count . '" name="' . $field['name'] . '[]" type="checkbox" value="' . 
                            $key . '" ' . (in_array($key, $default) ? 'checked="checked"' : '') . ' /><label class="attr" for="chk_' . $field['name'] . '_' . $count . '">' . $v . '</label></li>'; 
                $count ++;
            }
        }
        else
        {
            $return .= 	'<li><input id="chk_' . $field['name'] . '" name="' . $field['name'] . '" type="checkbox" value="1" ' . 
                        ($default == 1 ? 'checked="checked"' : '') . ' /><label class="attr" for="chk_' . $field['name'] . '">' . $field['values'] . '</label></li>';
        }
        $return .= '</ul>';
        return $return;	
    }
    ...
    {
        $_datetime = $field['values'] ? $field['values'] : 'yyyy-MM-dd HH:mm:ss';
        return '<input class="Wdate" style="width:150px;" type="text" name="' . $field['name'] . '" id="' .
        $field['name'] . '" value="' . $default . '" onFocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:\''.$_datetime.'\'})"/>';
    }
	...
	private function _content($field, $default)
	{
		...
		$CI = & get_instance();
		if ( ! $CI->platform->cache_exists(CMS_SHARE_PATH . 'settings/model/'.$options[0].'.php'))
		{
			return '内存模型不存在!';
		}
	...
	private function _linked_menu($field , $default)
	{
		...
		$ci = & get_instance();
		if ( ! $ci->platform->cache_exists(CMS_SHARE_PATH . 'settings/category/data_'.$options[0].'.php'))
		{
			return '分类模型数据不存在!';
		}
	...
	private function _get_data_from_model( & $field , $need_level = FALSE)
	{
		...
		$ci = & get_instance();
		if ( ! $ci->platform->cache_exists(CMS_SHARE_PATH . 'settings/category/data_' . $options[0] . EXT))
		{
			return FALSE;
		}
		$ci->settings->load('category/data_' . $options[0]);
		...
		foreach ($model_data[$options[0]] as $v)
		{
			...
			$need_level AND $field['levels'][$v['id']] = $v['deep'];
		}
		return TRUE;
	}
