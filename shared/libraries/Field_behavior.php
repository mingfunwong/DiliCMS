...
class Field_behavior
{
	...
	private function _load_extra_field($type)
	{
		if ( ! in_array($type, array_keys($this->_extra_fields)))
		{
			...
			if (file_exists(CMS_EXTENSION_PATH . 'fields/' . $extra_class . '.php'))
			{
				include CMS_EXTENSION_PATH . 'fields/' . $extra_class . '.php';
		...
		else
		{
			switch ($data['type'])
			{
				...
				case 'float' : $field = array(
								...
								break;
				case 'double' : $field = array(
												'type' => 'DOUBLE',
												 'constraint' => $data['length'] ? $data['length'] : 20,
												 'default' => 0
												) ;
								break;
				case 'input' : 
    ...
    public function on_list($field, $value)
    {
        ...
        else
        {
            switch ($field['type'])
            {
                case 'radio' 	:
                case 'select'	:
                            echo isset($field['values'][$value->$field['name']]) ?  $field['values'][$value->$field['name']] : '' ;
                            break;
                case 'checkbox' :
                            foreach (explode(',', $value->$field['name']) as $t)
                            {
                                echo isset($field['values'][$t]) ?  $field['values'][$t] . '<br />' : '' . '<br />';
                            }
                            break;
                case 'radio_from_model':
                case 'select_from_model':
                            ...
                            echo isset($setting[$options[0]][$value->$field['name']][$options[1]]) ? $setting[$options[0]][$value->$field['name']][$options[1]] : '' ;
                            break;
                case 'checkbox_from_model':
                            $options = explode('|', $field['values']);
                            $this->_ci->settings->load('category/data_' . $options[0]);
                            ...
                            {
                                echo isset($setting[$options[0]][$checkbox][$options[1]]) ? $setting[$options[0]][$checkbox][$options[1]].'<br />' : '<br />' ;
                            }
                            break;
                case 'linked_menu':
                            $options = explode('|', $field['values']);
                            ...
                            foreach ($temp_out as & $t)
                            {
                                ...
                                foreach ($temp as & $tt)
                                {
                                    $tt = (isset($setting[$options[0]][$tt][$options[1]]) ? $setting[$options[0]][$tt][$options[1]] : '');
                                }
                                $t = implode('-', $temp);
                            }
