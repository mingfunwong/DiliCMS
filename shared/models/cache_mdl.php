...
class Cache_mdl extends CI_Model
{
    ...
    private function _create_folder($folder = '')
    {
    	...
    	{
    		if ( ! file_exists(CMS_SHARE_PATH . 'settings/' . $folder))
	    	{
	    		mkdir(CMS_SHARE_PATH . 'settings/' . $folder);
	    	}
    	}
	...
	public function update_model_cache($target = '') 
	{
		...
		foreach ($models as $model)
		{
			...
			$this->platform->cache_write(CMS_SHARE_PATH . 'settings/model/'.$model['name'].'.php',
	...
	public function update_category_cache($target = '')
	{
		...
		foreach ($models as $model)
		{
			...
			$this->platform->cache_write(CMS_SHARE_PATH . 'settings/category/cate_' . $model['name'] . '.php', 
			...
			$this->platform->cache_write(CMS_SHARE_PATH . 'settings/category/data_' . $model['name'] . '.php', 
	...
	public function update_menu_cache()
	{
		...
		$this->platform->cache_write(CMS_SHARE_PATH . 'settings/menus.php', 
	...
	public function update_role_cache($target = '')
	{
		...
		{	
			...
			$this->platform->cache_write(CMS_SHARE_PATH . 'settings/acl/role_' . $role['id'] . '.php', 
	...
	public function update_site_cache()
	{
		...
		$this->platform->cache_write(CMS_SHARE_PATH . 'settings/site.php', 
	...
	public function update_backend_cache()
	{
		...
		$this->platform->cache_write(CMS_SHARE_PATH . 'settings/backend.php', 
									 array_to_cache("setting", $data));	
	}
	...
	public function update_plugin_cache()
	{
		...
		if ($plugins)
	    {
			foreach ($plugins as $key => $plugin)
			{
			    ...
			    $this_plugin_path = CMS_EXTENSION_PATH.'plugins/'.$plugin['name'].'/';
		...
		$this->platform->cache_write(CMS_SHARE_PATH . 'settings/plugins.php', 
	...
	public function update_fieldtypes_cache()
	{
		...
		$extra_path = CMS_EXTENSION_PATH . 'fields/';
		...
		$this->platform->cache_write(CMS_SHARE_PATH . 'settings/fieldtypes.php', 
