...
class Category_mdl extends CI_Model
{
	...
	public function get_all($model, $level = -1)
	{
		...
		{
			$row['children'] = $this->get_child_num($row['id'], $model);
			$records[] = $row;
		}
		return $records;
	}
	...
	public function add_new_category($data)
	{
		if ($this->db->insert($this->db->dbprefix('cate_models'), $data))
		{
			...
			$this->dbforge->add_field(array('id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE, 'auto_increment' => TRUE)));
	...
	public function edit_category_model($target_model, $data)
	{
		if ($this->db->where('id', $target_model->id)->update($this->db->dbprefix('cate_models'), $data))
		{
			...
			if ($old_table_name != $data['name'])
			{
				...
				$this->platform->cache_delete(CMS_SHARE_PATH . 'settings/category/cate_' . $old_table_name . '.php');
				$this->platform->cache_delete(CMS_SHARE_PATH . 'settings/category/data_' . $old_table_name . '.php');
			}
			return TRUE;
		}
		return FALSE;
	...
	public function del_category_model($model)
	{
		...
		{
			$this->platform->file_delete(CMS_SHARE_PATH . '../' . setting('attachment_dir') . '/' . $attachment->folder . '/' . $attachment->name . '.' . $attachment->type);		
		}
		...
		//清除缓存文件
		$this->platform->cache_delete(CMS_SHARE_PATH.'settings/category/cate_' . $model->name . '.php');
		$this->platform->cache_delete(CMS_SHARE_PATH.'settings/category/data_' . $model->name . '.php');
	}
