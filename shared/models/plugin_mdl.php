...
class Plugin_mdl extends CI_Model
{
	...
	public function export_plugin($ids)
	{
		...
		foreach ($plugins as $plugin)
		{
			...
			if ($this->platform->get_type() == 'default' && file_exists(CMS_EXTENSION_PATH . 'plugins/' . $plugin->name))
			{
				file_put_contents(CMS_EXTENSION_PATH . 'plugins/' . $plugin->name .'/plugin_' . $plugin->name . '_install.xml', $xml);
			}
		}
	}
