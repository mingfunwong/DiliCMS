...
class Install extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if (is_installed())
        {
            ...
            echo 'CMS已经安装过,要重新安装请先删除install.lock';
        ...
        if ($step == 'check')
        {
            if ( ! mysql_errno())
            {
                ...
                if ( ! is_sae())
                {
                    ...
                    //创建database.php
                    $database_config_file = BASEPATH.'../application/config/database.php';
                    if (! file_exists($database_config_file) and ! touch($database_config_file)) {
                        echo json_encode(array(
                            'status' => 0,
                            'messages' => array('额，没有权限创建application/config/database.php文件!')
                        ));
                        return;
                    }
                    if (FALSE === file_put_contents($database_config_file, $database_config)) {
                        echo json_encode(array(
                            'status' => 0,
                            'messages' => array('额，没有权限将数据写入application/config/database.php文件!')
                        ));
                        return;
                    }
                }
    ...
    public function complete()
    {
        //更新缓存
        define('PLATFORM', is_sae() ? 'sae' : 'default');
        define('CMS_SHARED_PATH', BASEPATH.'../shared/');
        $this->load->add_package_path(CMS_SHARED_PATH);
        ...
        if (is_sae())
        {
            $this->platform->file_write('install.lock', 'Welcome to CMS!');
        }
        else
        {
            $this->platform->file_write(CMS_SHARED_PATH.'settings/install.lock', 'Welcome to CMS!');
        }
    }
