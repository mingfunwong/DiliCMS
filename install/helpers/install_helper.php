...
if ( ! function_exists('is_memcache_ok'))
{
    function is_memcache_ok()
    {
        ...
        {
            memcache_set($mmc, "cms_install_test", "cms");
            return memcache_get($mmc, "cms_install_test") == 'cms';
        }
    }
}
...
if ( ! function_exists('is_storage_ok'))
{
    ...
    {
        $s = new SaeStorage();
        $status = $s->write('public', '.cms_install_test', '');
        $status AND $s->delete('public', '.cms_install_test');
        return $status;
    }
}
