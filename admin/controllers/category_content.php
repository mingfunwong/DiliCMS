...
class Category_content extends Admin_Controller
{
	...
	public function _view_post()
	{
		...
		$this->_check_permit();
		if ( ! $this->platform->cache_exists(CMS_SHARE_PATH . 'settings/category/cate_' . $model . '.php'))
		{
    ...
    private function _pagination($model)
    {
        ...
        if ($level != 0)
        {
            $data['parent'] = $this->db->where('id', $level)->get($this->db->dbprefix('u_c_') . $model['name'])->row();
	...
	public function _save_post()
	{
        ...
        if ($id)
        {
            ...
            $data['content'] = $this->db->where('id', $id)->get($this->db->dbprefix('u_c_') . $model)->row_array();
        ...
        if ($data['parentid'] > 0)
        {
            $current_level = $this->db->where('id', $data['parentid'])
        ...
        else
        {
            ...
            if ($data['parentid'] > 0)
            {
                ...
                $data['path'] = '{0}';
                $data['level'] = 1;
                $parent_class = $this->db->where('id', $data['parentid'])->get($this->db->dbprefix('u_c_') . $model)->row();
    ...
    public function _del_post()
    {
        ...
        if ($ids)
        {
            ...
            foreach ($ids as $v)
            {
                $_category = $this->db->select('path')->where('id', $v)->get($this->db->dbprefix('u_c_') . $model)->row();
                $where_string .= " OR path Like '" . $_category->path . ',{' . $v . "}%'";
            }
            $this->db->select('id')->from($this->db->dbprefix('u_c_') . $model);
            ...
            {
                $this->platform->file_delete(CMS_SHARE_PATH . '../' . 
            ...
            $this->db->where_in('id', $ids)->delete($this->db->dbprefix('u_c_') . $model);
            $this->plugin_manager->trigger('deleted', $ids);
            if (isset($model_data->auto_update)) {
                update_cache('category', $model);
            }
		}
