...
class Plugin extends Admin_Controller
{
	...
	private function _validate_model_form($name = '', $load_form = TRUE)
	{
		...
		$this->form_validation->set_rules('name', '插件标识', 'trim|required|alpha_dash|min_length[2]|max_length[20]' . $callback);
	...
	public function _import_post()
	{
		...
		try{ 
			...
			if ((string) $plugin_xml->attributes() === 'CMS' AND ($plugin_info = (array) $plugin_xml->plugin[0]))
			{
				$_POST = $plugin_info;
