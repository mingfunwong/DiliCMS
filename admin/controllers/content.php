...
class Content extends Admin_Controller
{
    ...
    public function down(){
        ...
        $pass_fields = array(
            ...
            'users' => explode(' ', 'id create_time phone nickname sex age country province city address'), // 用户列表
            'order' => explode(' ', 'id create_time user_id trade_no price status pay_type pay_status content pay_datetime confirm_datetime'), // 订单列表
        );
        foreach($data['model']['fields'] as $key => $val) {
            ...
            $fields[] = iconv('utf-8','GB2312//IGNORE',$val['description']);
        }
        ...
        foreach($data['provider']['list'] as $key => $val) {
            ...
            $array[] = iconv('utf-8','GB2312//IGNORE',implode(',', $list));
        }
        header("Content-Type: application/force-download");
        header("Content-Disposition: attachment; filename=".$model.date('YmdHi').".csv");
        echo implode("\n\t", $array);
        exit();
    }
