...
class Category extends Admin_Controller
{
	...
	private function _validate_model_form($name = '')
	{
		...
		$this->form_validation->set_rules('name', '分类模型标识', 'trim|required|alpha_dash|min_length[2]|max_length[20]' . $callback);
	...
	{
		if ($name == 'id' OR $name == 'parentid' OR $name == 'path' OR $name == 'level')
		{
			$this->form_validation->set_message('_check_field_name_valid', '字段标识不能为id或者parentid,或者path或者level！');	
			return FALSE;
		}
		else
		{
